...
class MyVpnService : VpnService() {
    ...
    private fun updateVpnState(forceFreeze: Boolean = false) {
        ...
        if (vpnInterface != null && isCurrentlyFrozen == anyBlockActive) {
            return
        }
        ...
        try {
            ...
            if (anyBlockActive) {
                // To freeze/block internet traffic, route everything to our local dummy interface
                builder.addRoute("0.0.0.0", 0)
            } else {
                // To let normal traffic flow, only route an unused private address so general internet still works!
